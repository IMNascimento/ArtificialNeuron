...
# Função para identificar se a frase contém um número
def contains_number(sentence):
    return bool(re.search(r'\d+(\.\d+)?', sentence))

# Função para identificar se a frase contém um e-mail
def contains_email(sentence):
    return bool(re.search(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b', sentence))
...
sentences = [
    ...
    "meu email é exemplo@dominio.com", "o número é 123", "quantidade é 45.67"
]
...
for sentence in sentences:
    label = [
        1 if 'oi' in sentence.lower() else 0,
        1 if 'você' in sentence.lower() else 0,
        1 if contains_number(sentence) else 0,
        1 if contains_email(sentence) else 0
    ]
    labels.append(label)
...
output_size = 4  # Quatro neurônios na camada de saída: "oi", "você", "número", "email"
...
test_phrases = ["oi você", "bom dia para todos", "oi como vai", "até logo", "oi meu caro", "meu email é exemplo@dominio.com", "o número é 123", "quantidade é 45.67"]
...
while True:
    ...
    print(f'Predição: "oi": {"Sim" if prediction[0] >= 0.5 else "Não"}, "você": {"Sim" if prediction[1] >= 0.5 else "Não"}, "número": {"Sim" if prediction[2] >= 0.5 else "Não"}, "email": {"Sim" if prediction[3] >= 0.5 else "Não"} - Valores: {prediction}')